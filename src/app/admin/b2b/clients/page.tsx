...
import { getClients, approveClient, rejectClient, deleteClient, initializeB2BStore, createApprovedClientAccount } from '@/lib/b2b-store';
import type { B2BClient, RegistrationStatus, BusinessType } from '@/types/b2b';
...
export default function AdminB2BClients() {
  ...
  const [createdAccount, setCreatedAccount] = useState<{ companyName: string; email: string; temporaryPassword: string } | null>(null);
  ...
  async function handleCreateClient(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await createApprovedClientAccount({
        companyName: String(formData.get('companyName') || ''),
        contactPerson: String(formData.get('contactPerson') || ''),
        email: String(formData.get('email') || ''),
        phone: String(formData.get('phone') || ''),
        country: String(formData.get('country') || ''),
        businessType: String(formData.get('businessType') || 'fashion_brand') as BusinessType,
      ...
      setCreateError(error instanceof Error ? error.message : 'Musteri hesabi olusturulamadi.');
    } finally {
      setCreating(false);
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Musteri hesaplarini yonetin ve firmalara portal giris bilgisi olusturun.</p>
        </div>
        <button
      ...
      {createdAccount && (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <p className="text-sm font-semibold text-emerald-900">Hesap olusturuldu</p>
              <p className="mt-1 text-sm text-emerald-700">
                {createdAccount.companyName} icin portal giris bilgileri hazir. Bu bilgileri musteriyle paylasabilirsiniz.
              </p>
            </div>
            <button
              onClick={() => setCreatedAccount(null)}
              className="self-start rounded-lg border border-emerald-300 px-3 py-1.5 text-xs font-medium text-emerald-700 hover:bg-emerald-100"
            >
              Kapat
            </button>
          </div>
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="rounded-lg bg-white px-4 py-3">
              <p className="text-[11px] font-medium uppercase tracking-wider text-gray-400">E-posta</p>
              <p className="mt-1 text-sm font-medium text-gray-900">{createdAccount.email}</p>
            </div>
            <div className="rounded-lg bg-white px-4 py-3">
              <p className="text-[11px] font-medium uppercase tracking-wider text-gray-400">Gecici sifre</p>
              <p className="mt-1 text-sm font-medium text-gray-900">{createdAccount.temporaryPassword}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {showCreate && (
        <div className="rounded-xl border border-blue-100 bg-white p-5 shadow-sm">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-sm font-semibold text-gray-900">Yeni Musteri Hesabi</h2>
              <p className="mt-1 text-sm text-gray-500">
                Firma hesabini sen olustur, sistem onayli kullanici olarak kaydetsin ve gecici sifre uretsin.
              </p>
            </div>
            <button
          ...
          <form onSubmit={handleCreateClient} className="mt-5 space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="md:col-span-2">
                <label className="mb-1.5 block text-sm font-medium text-gray-700">Firma adi</label>
                <input name="companyName" required className="w-full rounded-lg border border-gray-200 px-3.5 py-2.5 text-sm focus:border-blue-400 focus:outline-none" placeholder="Orn. NordMode" />
              </div>
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700">Yetkili kisi</label>
                <input name="contactPerson" required className="w-full rounded-lg border border-gray-200 px-3.5 py-2.5 text-sm focus:border-blue-400 focus:outline-none" placeholder="Ad Soyad" />
              </div>
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700">E-posta</label>
                <input name="email" type="email" required className="w-full rounded-lg border border-gray-200 px-3.5 py-2.5 text-sm focus:border-blue-400 focus:outline-none" placeholder="firma@ornek.com" />
              </div>
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700">Telefon</label>
                <input name="phone" required className="w-full rounded-lg border border-gray-200 px-3.5 py-2.5 text-sm focus:border-blue-400 focus:outline-none" placeholder="+90 ..." />
              </div>
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700">Ulke</label>
                <input name="country" required className="w-full rounded-lg border border-gray-200 px-3.5 py-2.5 text-sm focus:border-blue-400 focus:outline-none" placeholder="Ulke" />
              </div>
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700">Musteri tipi</label>
                <select name="businessType" className="w-full rounded-lg border border-gray-200 px-3.5 py-2.5 text-sm focus:border-blue-400 focus:outline-none">
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700">Gecici sifre</label>
                <input name="temporaryPassword" className="w-full rounded-lg border border-gray-200 px-3.5 py-2.5 text-sm focus:border-blue-400 focus:outline-none" placeholder="Bos birakirsan sistem uretir" />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {creating ? 'Olusturuluyor...' : 'Hesabi Olustur'}
              </button>
            </div>
          </form>
        </div>
