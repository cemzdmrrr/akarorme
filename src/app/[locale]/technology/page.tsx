...
const moduleHrefs = ['viewer', 'patterns', 'dashboard'] as const;

function parseTags(value: string, fallback: string[]): string[] {
  const tags = value
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean);
...
export default async function TechnologyPage({
  ...
  const technologyPage = getPageBySlug(pages, 'technology');
  const modules = dict.technology.modules.map((module, index) => ({
    title: getPageSectionContent(technologyPage, `module_${index + 1}_title`, params.locale, module.title),
    description: getPageSectionContent(
      technologyPage,
      `module_${index + 1}_description`,
      params.locale,
      module.description,
    ),
    tags: parseTags(
  ...
  return (
    <>
      ...
      <main>
        <PageHero
          title={getPageSectionContent(technologyPage, 'hero_title', params.locale, dict.technology.heroTitle)}
          highlight={getPageSectionContent(
            technologyPage,
            'hero_highlight',
            params.locale,
            dict.technology.heroHighlight,
          )}
          ...
          <div className="container-xl">
            <RevealOnScroll>
              <div className="mx-auto mb-16 max-w-2xl text-center">
                <h2 className="font-display text-3xl font-bold text-brand-dark md:text-4xl">
                  {getPageSectionContent(technologyPage, 'section_title', params.locale, dict.technology.sectionTitle)}{' '}
                  <span className="accent">
                    {getPageSectionContent(
                      technologyPage,
                      'section_highlight',
                      params.locale,
                      dict.technology.sectionHighlight,
                    )}
                  </span>
                </h2>
            ...
            <StaggerContainer className="grid gap-6 md:grid-cols-3">
              {modules.map((module, index) => (
                <StaggerItem key={moduleHrefs[index]}>
                  <Link
                    href={`/${params.locale}/technology/${moduleHrefs[index]}`}
                  ...
                  >
                    <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-xl bg-brand-accent/10 text-brand-accent-dark transition-all duration-300 group-hover:scale-110 group-hover:bg-brand-accent/20">
                      {moduleIcons[index]}
                    </div>
                    <h3 className="mb-2 font-display text-xl font-bold text-brand-dark">{module.title}</h3>
                    <p className="mb-5 flex-1 text-sm leading-relaxed text-brand-grey">{module.description}</p>
